...
$addresses_delete = NULL; // Initialize page object first

class caddresses_delete extends caddresses {
	...
	// Page object name
	var $PageObjName = 'addresses_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("addresseslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in addresses class, addressesinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("addresseslist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($addresses_delete)) $addresses_delete = new caddresses_delete();

// Page init
$addresses_delete->Page_Init();

// Page main
$addresses_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$addresses_delete->Page_Render();
?>
...
var CurrentForm = faddressesdelete = new ew_Form("faddressesdelete", "delete");

// Form_CustomValidate event
faddressesdelete.Form_CustomValidate = 
...
<?php $addresses_delete->ShowPageHeader(); ?>
<?php
$addresses_delete->ShowMessage();
?>
<form name="faddressesdelete" id="faddressesdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($addresses_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $addresses_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="addresses">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($addresses_delete->RecKeys as $key) { ?>
...
<?php } ?>
	...
	<tbody>
<?php
$addresses_delete->RecCnt = 0;
$i = 0;
while (!$addresses_delete->Recordset->EOF) {
	$addresses_delete->RecCnt++;
	$addresses_delete->RowCnt++;

	// Set row properties
	$addresses->ResetAttrs();
	$addresses->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$addresses_delete->LoadRowValues($addresses_delete->Recordset);
...
<?php
	$addresses_delete->Recordset->MoveNext();
}
$addresses_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $addresses_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
faddressesdelete.Init();
</script>
<?php
$addresses_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$addresses_delete->Page_Terminate();
?>
