...
$addresses_view = NULL; // Initialize page object first

class caddresses_view extends caddresses {
	...
	// Page object name
	var $PageObjName = 'addresses_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "addresseslist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "addresseslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_addresses\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_addresses',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.faddressesview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($addresses_view)) $addresses_view = new caddresses_view();

// Page init
$addresses_view->Page_Init();

// Page main
$addresses_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$addresses_view->Page_Render();
?>
...
var CurrentForm = faddressesview = new ew_Form("faddressesview", "view");

// Form_CustomValidate event
faddressesview.Form_CustomValidate = 
...
<?php if ($addresses->Export == "") { ?>
<div class="ewToolbar">
<?php $addresses_view->ExportOptions->Render("body") ?>
<?php
	foreach ($addresses_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php $addresses_view->ShowPageHeader(); ?>
<?php
$addresses_view->ShowMessage();
?>
<form name="faddressesview" id="faddressesview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($addresses_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $addresses_view->Token ?>">
<?php } ?>
