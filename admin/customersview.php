...
class ccustomers_view extends ccustomers {
	...
	function SetupOtherOptions() {
		...
		if ($GLOBALS["addresses_grid"] && $GLOBALS["addresses_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'addresses')) {
			$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=addresses")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailEditLink")) . "</a></li>";
			if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
			$DetailEditTblVar .= "addresses";
		}
		if ($GLOBALS["addresses_grid"] && $GLOBALS["addresses_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'addresses')) {
			$links .= "<li><a class=\"ewRowLink ewDetailCopy\" data-action=\"add\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailCopyLink")) . "\" href=\"" . ew_HtmlEncode($this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=addresses")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailCopyLink")) . "</a></li>";
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'addresses');
		if ($item->Visible) {
	...
	function ExportData() {
		...
		// Export detail records (addresses)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("addresses", explode(",", $this->getCurrentDetailTable()))) {
			global $addresses;
			if (!isset($addresses)) $addresses = new caddresses;
			$rsdetail = $addresses->LoadRs($addresses->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$oldtbl = $Doc->Table;
					$Doc->Table = $addresses;
					$addresses->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
					$Doc->Table = $oldtbl;
				}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("addresses", $DetailTblVar)) {
				...
				if ($GLOBALS["addresses_grid"]->DetailView) {
					$GLOBALS["addresses_grid"]->CurrentMode = "view";
...
<?php
	if (in_array("addresses", explode(",", $customers->getCurrentDetailTable())) && $addresses->DetailView) {
?>
