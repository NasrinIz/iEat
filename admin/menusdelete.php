...
$menus_delete = NULL; // Initialize page object first

class cmenus_delete extends cmenus {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("menuslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in menus class, menusinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("menuslist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($menus_delete)) $menus_delete = new cmenus_delete();

// Page init
$menus_delete->Page_Init();

// Page main
$menus_delete->Page_Main();
...
var CurrentForm = fmenusdelete = new ew_Form("fmenusdelete", "delete");

// Form_CustomValidate event
fmenusdelete.Form_CustomValidate = 
...
fmenusdelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
?>
<form name="fmenusdelete" id="fmenusdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($menus_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $menus_delete->Token ?>">
<?php } ?>
...
while (!$menus_delete->Recordset->EOF) {
	...
	// Set row properties
	$menus->ResetAttrs();
	$menus->RowType = EW_ROWTYPE_VIEW; // View
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $menus_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fmenusdelete.Init();
