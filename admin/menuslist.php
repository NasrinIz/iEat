...
$menus_list = NULL; // Initialize page object first

class cmenus_list extends cmenus {
	...
	// Page object name
	var $PageObjName = 'menus_list';

	// Grid form hidden field names
	var $FormName = 'fmenuslist';
	...
	function __construct() {
		...
		$this->MultiDeleteUrl = "menusdelete.php";
		$this->MultiUpdateUrl = "menusupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fmenuslistsrch";
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fmenuslistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fmenuslistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fmenuslistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fmenuslist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fmenuslistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($menus_list)) $menus_list = new cmenus_list();

// Page init
$menus_list->Page_Init();

// Page main
$menus_list->Page_Main();
...
var CurrentForm = fmenuslist = new ew_Form("fmenuslist", "list");
fmenuslist.FormKeyCountName = '<?php echo $menus_list->FormKeyCountName ?>';

// Form_CustomValidate event
fmenuslist.Form_CustomValidate = 
...
fmenuslist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
// Form object for search

var CurrentSearchForm = fmenuslistsrch = new ew_Form("fmenuslistsrch");
</script>
<script type="text/javascript">
...
<?php
	$bSelectLimit = $menus_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($menus_list->TotalRecs <= 0)
			$menus_list->TotalRecs = $menus->ListRecordCount();
	} else {
		if (!$menus_list->Recordset && ($menus_list->Recordset = $menus_list->LoadRecordset()))
			$menus_list->TotalRecs = $menus_list->Recordset->RecordCount();
	}
	$menus_list->StartRec = 1;
	if ($menus_list->DisplayRecs <= 0 || ($menus->Export <> "" && $menus->ExportAll)) // Display all records
		$menus_list->DisplayRecs = $menus_list->TotalRecs;
	if (!($menus->Export <> "" && $menus->ExportAll))
		$menus_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$menus_list->Recordset = $menus_list->LoadRecordset($menus_list->StartRec-1, $menus_list->DisplayRecs);

	// Set no record found message
	if ($menus->CurrentAction == "" && $menus_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$menus_list->setWarningMessage(ew_DeniedMsg());
		if ($menus_list->SearchWhere == "0=101")
			$menus_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$menus_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$menus_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($menus->Export == "" && $menus->CurrentAction == "") { ?>
<form name="fmenuslistsrch" id="fmenuslistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($menus_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fmenuslistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="menus">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($menus_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($menus_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $menus_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($menus_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($menus_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($menus_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($menus_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<div class="box ewBox ewGrid<?php if ($menus_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> menus">
<div class="box-header ewGridUpperPanel">
<?php if ($menus->CurrentAction <> "gridadd" && $menus->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($menus_list->Pager)) $menus_list->Pager = new cPrevNextPager($menus_list->StartRec, $menus_list->DisplayRecs, $menus_list->TotalRecs, $menus_list->AutoHidePager) ?>
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $menus_list->PageUrl() ?>start=<?php echo $menus_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($menus_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $menus_list->PageUrl() ?>start=<?php echo $menus_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $menus_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($menus_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $menus_list->PageUrl() ?>start=<?php echo $menus_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($menus_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $menus_list->PageUrl() ?>start=<?php echo $menus_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<option value="30"<?php if ($menus_list->DisplayRecs == 30) { ?> selected<?php } ?>>30</option>
...
<?php
	foreach ($menus_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
</div>
<form name="fmenuslist" id="fmenuslist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($menus_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $menus_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="menus">
<div id="gmp_menus" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($menus_list->TotalRecs > 0 || $menus->CurrentAction == "gridedit") { ?>
<table id="tbl_menuslist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$menus_list->RowType = EW_ROWTYPE_HEADER;
...
<tbody>
<?php
if ($menus->ExportAll && $menus->Export <> "") {
...
if ($menus_list->Recordset && !$menus_list->Recordset->EOF) {
	...
	$bSelectLimit = $menus_list->UseSelectLimit;
	if (!$bSelectLimit && $menus_list->StartRec > 1)
...
$menus->RowType = EW_ROWTYPE_AGGREGATEINIT;
$menus->ResetAttrs();
$menus_list->RenderRow();
while ($menus_list->RecCnt < $menus_list->StopRec) {
	$menus_list->RecCnt++;
	if (intval($menus_list->RecCnt) >= intval($menus_list->StartRec)) {
		$menus_list->RowCnt++;
		...
		$menus->ResetAttrs();
		$menus->CssClass = "";
		if ($menus->CurrentAction == "gridadd") {
		} else {
			$menus_list->LoadRowValues($menus_list->Recordset); // Load row values
		}
		$menus->RowType = EW_ROWTYPE_VIEW; // Render view
