...
class cmenus_view extends cmenus {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["MenuID"] <> "") {
			$this->RecKey["MenuID"] = $_GET["MenuID"];
			$KeyUrl .= "&amp;MenuID=" . urlencode($this->RecKey["MenuID"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["MenuID"] <> "") {
				...
				$this->RecKey["MenuID"] = $this->MenuID->QueryStringValue;
			} elseif (@$_POST["MenuID"] <> "") {
				$this->MenuID->setFormValue($_POST["MenuID"]);
				$this->RecKey["MenuID"] = $this->MenuID->FormValue;
			} else {
	...
	function SetupOtherOptions() {
		...
		// "detail_sub_menus"
		$item = &$option->Add("detail_sub_menus");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("sub_menus", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["sub_menus_grid"] && $GLOBALS["sub_menus_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'sub_menus')) {
			$links .= "<li><a class=\"ewRowLink ewDetailView\" data-action=\"view\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailViewLink")) . "\" href=\"" . ew_HtmlEncode($this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=sub_menus")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailViewLink")) . "</a></li>";
			if ($DetailViewTblVar <> "") $DetailViewTblVar .= ",";
			$DetailViewTblVar .= "sub_menus";
		}
		if ($GLOBALS["sub_menus_grid"] && $GLOBALS["sub_menus_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'sub_menus')) {
			$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=sub_menus")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailEditLink")) . "</a></li>";
			if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
			$DetailEditTblVar .= "sub_menus";
		}
		if ($GLOBALS["sub_menus_grid"] && $GLOBALS["sub_menus_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'sub_menus')) {
			$links .= "<li><a class=\"ewRowLink ewDetailCopy\" data-action=\"add\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailCopyLink")) . "\" href=\"" . ew_HtmlEncode($this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=sub_menus")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailCopyLink")) . "</a></li>";
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'sub_menus');
		if ($item->Visible) {
	...
	function ExportData() {
		...
		// Export detail records (sub_menus)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("sub_menus", explode(",", $this->getCurrentDetailTable()))) {
			global $sub_menus;
			if (!isset($sub_menus)) $sub_menus = new csub_menus;
			$rsdetail = $sub_menus->LoadRs($sub_menus->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$oldtbl = $Doc->Table;
					$Doc->Table = $sub_menus;
					$sub_menus->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
					$Doc->Table = $oldtbl;
				}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("sub_menus", $DetailTblVar)) {
				...
				if ($GLOBALS["sub_menus_grid"]->DetailView) {
					$GLOBALS["sub_menus_grid"]->CurrentMode = "view";
...
<?php if ($menus->MenuID->Visible) { // MenuID ?>
	<tr id="r_MenuID">
		<td class="col-sm-2"><span id="elh_menus_MenuID"><?php echo $menus->MenuID->FldCaption() ?></span></td>
		<td data-name="MenuID"<?php echo $menus->MenuID->CellAttributes() ?>>
<span id="el_menus_MenuID">
<span<?php echo $menus->MenuID->ViewAttributes() ?>>
<?php echo $menus->MenuID->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
	if (in_array("sub_menus", explode(",", $menus->getCurrentDetailTable())) && $sub_menus->DetailView) {
?>
