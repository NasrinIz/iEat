...
$order_details_grid = NULL; // Initialize page object first

class corder_details_grid extends corder_details {
	...
	// Page object name
	var $PageObjName = 'order_details_grid';

	// Grid form hidden field names
	var $FormName = 'forder_detailsgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["order_details"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->order_detail_id->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_menu_id") && $objForm->HasValue("o_menu_id") && $this->menu_id->CurrentValue <> $this->menu_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_sub_menu_id") && $objForm->HasValue("o_sub_menu_id") && $this->sub_menu_id->CurrentValue <> $this->sub_menu_id->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->order_detail_id->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->sub_menu_id->setOldValue($objForm->GetValue("o_sub_menu_id"));
		...
		if (!$this->order_detail_id->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->order_detail_id->CurrentValue = strval($arKeys[0]); // order_detail_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
