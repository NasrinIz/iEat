...
$orderdetails_grid = NULL; // Initialize page object first

class corderdetails_grid extends corderdetails {
	...
	// Page object name
	var $PageObjName = 'orderdetails_grid';

	// Grid form hidden field names
	var $FormName = 'forderdetailsgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["orderdetails"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->OrderDetailID->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_Quantity") && $objForm->HasValue("o_Quantity") && $this->Quantity->CurrentValue <> $this->Quantity->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_MenuID") && $objForm->HasValue("o_MenuID") && $this->MenuID->CurrentValue <> $this->MenuID->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_SubMenu") && $objForm->HasValue("o_SubMenu") && $this->SubMenu->CurrentValue <> $this->SubMenu->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->OrderDetailID->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->MenuID->setOldValue($objForm->GetValue("o_MenuID"));
		if (!$this->SubMenu->FldIsDetailKey) {
			$this->SubMenu->setFormValue($objForm->GetValue("x_SubMenu"));
		}
		$this->SubMenu->setOldValue($objForm->GetValue("o_SubMenu"));
		...
		if (!$this->OrderDetailID->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->OrderDetailID->CurrentValue = strval($arKeys[0]); // OrderDetailID
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->OrderID->getSessionValue() <> "") {
			$rsnew['OrderID'] = $this->OrderID->getSessionValue();
		}
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "orders") {
			...
			if ($GLOBALS["orders"]->EventCancelled) $this->EventCancelled = TRUE;
		}
