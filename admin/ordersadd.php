...
class corders_add extends corders {
	...
	function LoadDefaultValues() {
		...
		$this->TotalPrice->CurrentValue = NULL;
		$this->TotalPrice->OldValue = $this->TotalPrice->CurrentValue;
		$this->PaymentTypeID->CurrentValue = NULL;
		$this->PaymentTypeID->OldValue = $this->PaymentTypeID->CurrentValue;
		$this->DeliveryTypeID->CurrentValue = NULL;
		$this->DeliveryTypeID->OldValue = $this->DeliveryTypeID->CurrentValue;
		$this->Description->CurrentValue = NULL;
		$this->Description->OldValue = $this->Description->CurrentValue;
		$this->FeedBack->CurrentValue = NULL;
		$this->FeedBack->OldValue = $this->FeedBack->CurrentValue;
		$this->OrderDateTime->CurrentValue = NULL;
		$this->OrderDateTime->OldValue = $this->OrderDateTime->CurrentValue;
	}
	...
	function NewRow() {
		...
		$row['TotalPrice'] = $this->TotalPrice->CurrentValue;
		$row['PaymentTypeID'] = $this->PaymentTypeID->CurrentValue;
		$row['DeliveryTypeID'] = $this->DeliveryTypeID->CurrentValue;
		$row['Description'] = $this->Description->CurrentValue;
		$row['FeedBack'] = $this->FeedBack->CurrentValue;
		$row['OrderDateTime'] = $this->OrderDateTime->CurrentValue;
		return $row;
	}
	...
	function ValidateForm() {
		...
		if (in_array("orderdetails", $DetailTblVar) && $GLOBALS["orderdetails"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// TotalPrice
		$this->TotalPrice->SetDbValueDef($rsnew, $this->TotalPrice->CurrentValue, 0, FALSE);

		// PaymentTypeID
		$this->PaymentTypeID->SetDbValueDef($rsnew, $this->PaymentTypeID->CurrentValue, 0, FALSE);

		// DeliveryTypeID
		$this->DeliveryTypeID->SetDbValueDef($rsnew, $this->DeliveryTypeID->CurrentValue, 0, FALSE);
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("orderdetails", $DetailTblVar) && $GLOBALS["orderdetails"]->DetailAdd) {
				$GLOBALS["orderdetails"]->OrderID->setSessionValue($this->OrderID->CurrentValue); // Set master key
				if (!isset($GLOBALS["orderdetails_grid"])) $GLOBALS["orderdetails_grid"] = new corderdetails_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "orderdetails"); // Load user level of detail table
				$AddRow = $GLOBALS["orderdetails_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["orderdetails"]->OrderID->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("orderdetails", $DetailTblVar)) {
				...
				if ($GLOBALS["orderdetails_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["orderdetails_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["orderdetails_grid"]->CurrentMode = "add";
					$GLOBALS["orderdetails_grid"]->CurrentAction = "gridadd";
...
fordersadd.Lists["x_CustomerID"] = {"LinkField":"x_CustomerID","Ajax":true,"AutoFill":false,"DisplayFields":["x_FullName","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"customers"};
fordersadd.Lists["x_CustomerID"].Data = "<?php echo $orders_add->CustomerID->LookupFilterQuery(FALSE, "add") ?>";
fordersadd.Lists["x_ProvinceID"] = {"LinkField":"x_ProvinceID","Ajax":true,"AutoFill":false,"DisplayFields":["x_Name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"provinces"};
fordersadd.Lists["x_ProvinceID"].Data = "<?php echo $orders_add->ProvinceID->LookupFilterQuery(FALSE, "add") ?>";
fordersadd.Lists["x_PaymentTypeID"] = {"LinkField":"x_PaymentTypeID","Ajax":true,"AutoFill":false,"DisplayFields":["x_Name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"paymenttypes"};
fordersadd.Lists["x_PaymentTypeID"].Data = "<?php echo $orders_add->PaymentTypeID->LookupFilterQuery(FALSE, "add") ?>";
fordersadd.Lists["x_DeliveryTypeID"] = {"LinkField":"x_DeliveryTypeID","Ajax":true,"AutoFill":false,"DisplayFields":["x_Name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"deliverytypes"};
fordersadd.Lists["x_DeliveryTypeID"].Data = "<?php echo $orders_add->DeliveryTypeID->LookupFilterQuery(FALSE, "add") ?>";
...
<?php if ($orders->CustomerID->Visible) { // CustomerID ?>
	<div id="r_CustomerID" class="form-group">
		<label id="elh_orders_CustomerID" for="x_CustomerID" class="<?php echo $orders_add->LeftColumnClass ?>"><?php echo $orders->CustomerID->FldCaption() ?></label>
...
<?php if ($orders->FullName->Visible) { // FullName ?>
	<div id="r_FullName" class="form-group">
		<label id="elh_orders_FullName" for="x_FullName" class="<?php echo $orders_add->LeftColumnClass ?>"><?php echo $orders->FullName->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $orders_add->RightColumnClass ?>"><div<?php echo $orders->FullName->CellAttributes() ?>>
...
<?php if ($orders->ProvinceID->Visible) { // ProvinceID ?>
	<div id="r_ProvinceID" class="form-group">
		<label id="elh_orders_ProvinceID" for="x_ProvinceID" class="<?php echo $orders_add->LeftColumnClass ?>"><?php echo $orders->ProvinceID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $orders_add->RightColumnClass ?>"><div<?php echo $orders->ProvinceID->CellAttributes() ?>>
...
<?php if ($orders->ZipCode->Visible) { // ZipCode ?>
	<div id="r_ZipCode" class="form-group">
		<label id="elh_orders_ZipCode" for="x_ZipCode" class="<?php echo $orders_add->LeftColumnClass ?>"><?php echo $orders->ZipCode->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $orders_add->RightColumnClass ?>"><div<?php echo $orders->ZipCode->CellAttributes() ?>>
...
<?php if ($orders->TotalPrice->Visible) { // TotalPrice ?>
	<div id="r_TotalPrice" class="form-group">
		<label id="elh_orders_TotalPrice" for="x_TotalPrice" class="<?php echo $orders_add->LeftColumnClass ?>"><?php echo $orders->TotalPrice->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $orders_add->RightColumnClass ?>"><div<?php echo $orders->TotalPrice->CellAttributes() ?>>
...
<?php if ($orders->PaymentTypeID->Visible) { // PaymentTypeID ?>
	<div id="r_PaymentTypeID" class="form-group">
		<label id="elh_orders_PaymentTypeID" for="x_PaymentTypeID" class="<?php echo $orders_add->LeftColumnClass ?>"><?php echo $orders->PaymentTypeID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $orders_add->RightColumnClass ?>"><div<?php echo $orders->PaymentTypeID->CellAttributes() ?>>
...
<?php if ($orders->DeliveryTypeID->Visible) { // DeliveryTypeID ?>
	<div id="r_DeliveryTypeID" class="form-group">
		<label id="elh_orders_DeliveryTypeID" for="x_DeliveryTypeID" class="<?php echo $orders_add->LeftColumnClass ?>"><?php echo $orders->DeliveryTypeID->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $orders_add->RightColumnClass ?>"><div<?php echo $orders->DeliveryTypeID->CellAttributes() ?>>
...
<?php
	if (in_array("orderdetails", explode(",", $orders->getCurrentDetailTable())) && $orderdetails->DetailAdd) {
?>
