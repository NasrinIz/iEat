...
class cprovinces extends cTable {
	...
	function __construct() {
		...
		// ProvinceID
		$this->ProvinceID = new cField('provinces', 'provinces', 'x_ProvinceID', 'ProvinceID', '`ProvinceID`', '`ProvinceID`', 3, -1, FALSE, '`ProvinceID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ProvinceID->Sortable = FALSE; // Allow sort
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->ProvinceID->setDbValue($conn->Insert_ID());
			$rs['ProvinceID'] = $this->ProvinceID->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('ProvinceID', $rs))
				ew_AddFilter($where, ew_QuotedName('ProvinceID', $this->DBID) . '=' . ew_QuotedValue($rs['ProvinceID'], $this->ProvinceID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->ProvinceID->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->ProvinceID->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@ProvinceID@", ew_AdjustSql($this->ProvinceID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "ProvinceID:" . ew_VarToJson($this->ProvinceID->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->ProvinceID->CurrentValue)) {
			$sUrl .= "ProvinceID=" . urlencode($this->ProvinceID->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["ProvinceID"]))
				$arKeys[] = $_POST["ProvinceID"];
			elseif (isset($_GET["ProvinceID"]))
				$arKeys[] = $_GET["ProvinceID"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->ProvinceID->CurrentValue = $key;
