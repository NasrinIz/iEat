...
$stores_delete = NULL; // Initialize page object first

class cstores_delete extends cstores {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("storeslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in stores class, storesinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("storeslist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($stores_delete)) $stores_delete = new cstores_delete();

// Page init
$stores_delete->Page_Init();

// Page main
$stores_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$stores_delete->Page_Render();
?>
...
var CurrentForm = fstoresdelete = new ew_Form("fstoresdelete", "delete");

// Form_CustomValidate event
fstoresdelete.Form_CustomValidate = 
...
<?php $stores_delete->ShowPageHeader(); ?>
<?php
$stores_delete->ShowMessage();
?>
<form name="fstoresdelete" id="fstoresdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($stores_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $stores_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="stores">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($stores_delete->RecKeys as $key) { ?>
...
<?php } ?>
	...
	<tbody>
<?php
$stores_delete->RecCnt = 0;
$i = 0;
while (!$stores_delete->Recordset->EOF) {
	$stores_delete->RecCnt++;
	$stores_delete->RowCnt++;

	// Set row properties
	$stores->ResetAttrs();
	$stores->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$stores_delete->LoadRowValues($stores_delete->Recordset);
...
<?php
	$stores_delete->Recordset->MoveNext();
}
$stores_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $stores_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fstoresdelete.Init();
