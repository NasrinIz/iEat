...
$stores_view = NULL; // Initialize page object first

class cstores_view extends cstores {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "storeslist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "storeslist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_stores\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_stores',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fstoresview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($stores_view)) $stores_view = new cstores_view();

// Page init
$stores_view->Page_Init();

// Page main
$stores_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$stores_view->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($stores->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "view";
var CurrentForm = fstoresview = new ew_Form("fstoresview", "view");

// Form_CustomValidate event
fstoresview.Form_CustomValidate = 
...
<?php if ($stores->Export == "") { ?>
<div class="ewToolbar">
<?php $stores_view->ExportOptions->Render("body") ?>
<?php
	foreach ($stores_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php $stores_view->ShowPageHeader(); ?>
<?php
$stores_view->ShowMessage();
?>
<form name="fstoresview" id="fstoresview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($stores_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $stores_view->Token ?>">
<?php } ?>
