...
$sub_menus_grid = NULL; // Initialize page object first

class csub_menus_grid extends csub_menus {
	...
	// Page object name
	var $PageObjName = 'sub_menus_grid';

	// Grid form hidden field names
	var $FormName = 'fsub_menusgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["sub_menus"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->SubMenuID->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->SubMenuID->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->Picture->Upload->Index = $this->RowIndex;
	...
	function LoadFormValues() {
		...
		if (!$this->SubMenuID->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->SubMenuID->CurrentValue = strval($arKeys[0]); // SubMenuID
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "menus") {
			...
			if ($GLOBALS["menus"]->EventCancelled) $this->EventCancelled = TRUE;
		}
