...
$timings_list = NULL; // Initialize page object first

class ctimings_list extends ctimings {
	...
	// Page object name
	var $PageObjName = 'timings_list';

	// Grid form hidden field names
	var $FormName = 'ftimingslist';
	...
	function __construct() {
		...
		$this->AddUrl = "timingsadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "timingsdelete.php";
		$this->MultiUpdateUrl = "timingsupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ftimingslistsrch";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ftimingslistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = FALSE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ftimingslistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = FALSE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ftimingslist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
...
// Create page object
if (!isset($timings_list)) $timings_list = new ctimings_list();

// Page init
$timings_list->Page_Init();

// Page main
$timings_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$timings_list->Page_Render();
?>
...
var CurrentForm = ftimingslist = new ew_Form("ftimingslist", "list");
ftimingslist.FormKeyCountName = '<?php echo $timings_list->FormKeyCountName ?>';

// Form_CustomValidate event
ftimingslist.Form_CustomValidate = 
...
<div class="ewToolbar">
<?php if ($timings_list->TotalRecs > 0 && $timings_list->ExportOptions->Visible()) { ?>
<?php $timings_list->ExportOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php
	$bSelectLimit = $timings_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($timings_list->TotalRecs <= 0)
			$timings_list->TotalRecs = $timings->ListRecordCount();
	} else {
		if (!$timings_list->Recordset && ($timings_list->Recordset = $timings_list->LoadRecordset()))
			$timings_list->TotalRecs = $timings_list->Recordset->RecordCount();
	}
	$timings_list->StartRec = 1;
	if ($timings_list->DisplayRecs <= 0 || ($timings->Export <> "" && $timings->ExportAll)) // Display all records
		$timings_list->DisplayRecs = $timings_list->TotalRecs;
	if (!($timings->Export <> "" && $timings->ExportAll))
		$timings_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$timings_list->Recordset = $timings_list->LoadRecordset($timings_list->StartRec-1, $timings_list->DisplayRecs);

	// Set no record found message
	if ($timings->CurrentAction == "" && $timings_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$timings_list->setWarningMessage(ew_DeniedMsg());
		if ($timings_list->SearchWhere == "0=101")
			$timings_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$timings_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$timings_list->RenderOtherOptions();
?>
<?php $timings_list->ShowPageHeader(); ?>
<?php
$timings_list->ShowMessage();
?>
<?php if ($timings_list->TotalRecs > 0 || $timings->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($timings_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> timings">
<div class="box-header ewGridUpperPanel">
<?php if ($timings->CurrentAction <> "gridadd" && $timings->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($timings_list->Pager)) $timings_list->Pager = new cPrevNextPager($timings_list->StartRec, $timings_list->DisplayRecs, $timings_list->TotalRecs, $timings_list->AutoHidePager) ?>
<?php if ($timings_list->Pager->RecordCount > 0 && $timings_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($timings_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $timings_list->PageUrl() ?>start=<?php echo $timings_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($timings_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $timings_list->PageUrl() ?>start=<?php echo $timings_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $timings_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($timings_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $timings_list->PageUrl() ?>start=<?php echo $timings_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($timings_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $timings_list->PageUrl() ?>start=<?php echo $timings_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $timings_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($timings_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $timings_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $timings_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $timings_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($timings_list->TotalRecs > 0 && (!$timings_list->AutoHidePageSizeSelector || $timings_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="timings">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="20"<?php if ($timings_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="30"<?php if ($timings_list->DisplayRecs == 30) { ?> selected<?php } ?>>30</option>
<option value="50"<?php if ($timings_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($timings->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($timings_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
</div>
<form name="ftimingslist" id="ftimingslist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($timings_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $timings_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="timings">
<div id="gmp_timings" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($timings_list->TotalRecs > 0 || $timings->CurrentAction == "gridedit") { ?>
<table id="tbl_timingslist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$timings_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$timings_list->RenderListOptions();
...
$timings_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($timings->ExportAll && $timings->Export <> "") {
	$timings_list->StopRec = $timings_list->TotalRecs;
} else {

	// Set the last record to display
	if ($timings_list->TotalRecs > $timings_list->StartRec + $timings_list->DisplayRecs - 1)
		$timings_list->StopRec = $timings_list->StartRec + $timings_list->DisplayRecs - 1;
	else
		$timings_list->StopRec = $timings_list->TotalRecs;
}
$timings_list->RecCnt = $timings_list->StartRec - 1;
if ($timings_list->Recordset && !$timings_list->Recordset->EOF) {
	$timings_list->Recordset->MoveFirst();
	$bSelectLimit = $timings_list->UseSelectLimit;
	if (!$bSelectLimit && $timings_list->StartRec > 1)
		$timings_list->Recordset->Move($timings_list->StartRec - 1);
} elseif (!$timings->AllowAddDeleteRow && $timings_list->StopRec == 0) {
	$timings_list->StopRec = $timings->GridAddRowCount;
}

// Initialize aggregate
$timings->RowType = EW_ROWTYPE_AGGREGATEINIT;
$timings->ResetAttrs();
$timings_list->RenderRow();
while ($timings_list->RecCnt < $timings_list->StopRec) {
	$timings_list->RecCnt++;
	if (intval($timings_list->RecCnt) >= intval($timings_list->StartRec)) {
		$timings_list->RowCnt++;

		// Set up key count
		$timings_list->KeyCount = $timings_list->RowIndex;

		// Init row class and style
		$timings->ResetAttrs();
		$timings->CssClass = "";
		if ($timings->CurrentAction == "gridadd") {
		} else {
			$timings_list->LoadRowValues($timings_list->Recordset); // Load row values
		}
		$timings->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$timings->RowAttrs = array_merge($timings->RowAttrs, array('data-rowindex'=>$timings_list->RowCnt, 'id'=>'r' . $timings_list->RowCnt . '_timings', 'data-rowtype'=>$timings->RowType));
		...
		$timings_list->RenderListOptions();
?>
	<tr<?php echo $timings->RowAttributes() ?>>
<?php
...
$timings_list->ListOptions->Render("body", "right", $timings_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($timings->CurrentAction <> "gridadd")
		$timings_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($timings->CurrentAction == "") { ?>
...
// Close recordset
if ($timings_list->Recordset)
	$timings_list->Recordset->Close();
?>
...
<?php } ?>
<?php if ($timings_list->TotalRecs == 0 && $timings->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($timings_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
